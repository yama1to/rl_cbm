...
### 共通設定
from maincbmgpu import Config
config = Config()
common.config  = config
common.prefix  = "data%s_maincbmgpu" % common.string_now() # 実験名（ファイルの接頭辞）
common.dir_path= "data/data%s_maincbmgpu " % common.string_now() # 実験データを出力するディレクトリのパス
common.exe     = "python3 maincbmgpu.py " # 実行されるプログラム
common.columns=['dataset','seed','id','alpha_i','alpha_r','alpha_b','alpha_s','beta_i','beta_r','beta_b','cnt_overflow',"plus_reward_times"]
common.parallel= 1
common.setup()
...
def optimize():
    ...
    opt.maximize(target="plus_reward_times",iteration=10,population=10,samples=2)
...
def gridsearch(X1,min=0,max=1,num=41,samples=10):
    ...
    plt.subplot(2,1,1)
    x,ymean,ystd,ymin,ymax = vs.analyze(df,X1,"plus_reward_times")
    plot1(x,ymean,ystd,ymin,ymax,color=cmap(1),label="plus_reward_times")
    plt.ylabel("plus_reward_times")
...
def gs2():
    ...
    # gridsearch("alpha_s",min=0.01,max=2,num=41,samples=ns)
